...
class WaveformTester_DfcTileTester_twA(dut: DfcTile) extends PeekPokeTester(dut) {
  ...
  val opAddr_A_t =  Seq(0, 12, 12, 12, 0, 0, 0, 0)
...
class WaveformSpec_DfcTileTester_tA extends FlatSpec with Matchers{
...
class WaveformTester_DfcTileTester_twD(dut: DfcTile) extends PeekPokeTester(dut) {
  ...
  val wEnA_t =      Seq(0, 0, 0, 0, 0, 0, 0, 0, 0)
  //write TableD with 2 enable
  val wEnD_Addr_t = Seq(0, 0, 1, 0, 1, 0, 1, 0, 0)
  val wEnD_Info_t = Seq(0, 0, 0, 1, 0, 1, 0, 1, 0)

  val wData_t =     Seq(0, 0, wData_D_addr0, wData_D_info0, wData_D_addr1,
                        wData_D_info1, wData_D_addr2, wData_D_info2, 0)

  val opAddr_A_t =  Seq(0, 0, 0, 0, 0, 0, 0, 0, 0)
  //opAddrD write same addr
  val opAddr_D_t =  Seq(0, 0, 8, 8, 9, 9, 10, 10, 0)

  val listenAddr_t= Seq(0, 0, 0, 0, 0, 0, 0, 0, 0)
